	...
	<div class="view-stack">
		...
		<section class="planner-grid">
			<article v-for="day in days" :key="iso(day)" class="day-column panel" :class="{ today: iso(day) === iso(new Date()) }">
				...
				<div v-for="meal in meals.filter(item => item.date === iso(day))" :key="meal.id" class="meal-card"><small>{{ meal.slot }}</small><a :href="`#/recipes/${meal.recipeId}`">{{ meal.recipeTitle }}</a><span>{{ meal.servings }} {{ t('smartcook', 'servings') }}</span><button @click="remove(meal.id)">×</button></div>
