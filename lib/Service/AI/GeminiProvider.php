...
final class GeminiProvider implements AiProviderInterface {
    ...
    public function extractRecipe(string $text, string $language, array $config): array {
        ...
        $payload = [
            'contents' => [['parts' => [['text' => isset($config['assistant']) && is_array($config['assistant']) ? $this->prompts->assistant((string)($config['assistant']['question'] ?? ''), (array)($config['assistant']['recipes'] ?? []), $language) : (isset($config['planner']) && is_array($config['planner']) ? $this->prompts->mealPlan($config['planner']['recipes'] ?? [], (string)($config['planner']['from'] ?? ''), (string)($config['planner']['to'] ?? ''), (array)($config['planner']['preferences'] ?? [])) : (isset($config['refinement']) && is_array($config['refinement']) ? $this->prompts->refinement($config['refinement'], $language) : $this->prompts->recipe($text, $language)))]]]],
